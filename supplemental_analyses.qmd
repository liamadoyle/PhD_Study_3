...
I will address the potential order confound effect in several ways. To begin with, I will start by subsetting each participant's first five actions and their last five actions in the one-shot games and comparing them to determine their consistency. After that, I will use a paired samples *t*-test to determine if there is a significant difference between the mean defection rate in the first half vs. the second half.

### Mean Scores

In the first code chunk, I will examine the mean defection rates in the first and second half.

```{r confound defection means, echo=FALSE}
...
```{r confound average payoff means, echo=FALSE}
...
```{r confound relative payoff means, echo=FALSE}
...
```{r confound defection correlation, echo = FALSE}
...
```{r confound average payoff correlation, echo = FALSE}
...
```{r confound relative payoff correlation, echo = FALSE}
...
```{r confound lme defection, echo=FALSE}
...
model <- lmer(Decision ~ SRP_Total * Game_Type + (1 | Participant_ID), data = long_data)
...
```{r confound lme avg_payoff, echo=FALSE}
...
# Linear mixed model with 'Game_Type' as a factor (including both one-shot halves and iterated games)
model <- lmer(Average_Payoff ~ SRP_Total * Game_Type + (1 | Participant_ID), data = long_data)
...
# Linear mixed model with 'Game_Type' as a factor (including both one-shot halves and iterated games)
model <- lmer(Relative_Payoff ~ SRP_Total * Game_Type + (1 | Participant_ID), data = long_data)
